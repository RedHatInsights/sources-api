...
RSpec.describe("v3.0 - Authentications") do
  ...
  let(:collection_path) { "/api/v3.0/authentications" }
  ...
  describe("/api/v3.0/authentications") do
    context "get" do
    ...
    context "post" do
      it "success: with valid body" do
        ...
        expect(response).to have_attributes(
          :status => 201,
          :location => "http://www.example.com/api/v3.0/authentications/#{response.parsed_body["id"]}",
  ...
  describe("/api/v3.0/authentications/:id") do

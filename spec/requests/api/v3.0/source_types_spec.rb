...
RSpec.describe("v3.0 - SourceTypes") do
  include ::Spec::Support::TenantIdentity

  let(:headers)         { {"CONTENT_TYPE" => "application/json", "x-rh-identity" => identity} }
  let(:attributes)      { {"name" => "test_name", "product_name" => "Test Product", "vendor" => "TestVendor"} }
  let(:collection_path) { "/api/v3.0/source_types" }

  describe("/api/v3.0/source_types") do
    context "get" do
  ...
  describe("/api/v3.0/source_types/:id") do
  ...
  describe("/api/v3.0/source_types/:id/sources") do

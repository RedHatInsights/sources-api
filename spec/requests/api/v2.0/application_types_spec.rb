RSpec.describe("v2.0 - ApplicationTypes") do
  include ::Spec::Support::TenantIdentity

  let(:headers)         { {"CONTENT_TYPE" => "application/json", "x-rh-identity" => identity} }
  let(:attributes)      { {"name" => "my application type"} }
  let(:collection_path) { "/api/v2.0/application_types" }

  describe("/api/v2.0/application_types") do
    context "get" do
  ...
  describe("subcollections") do
    ...
    existing_subcollections.each do |subcollection|
      describe("/api/v2.0/application_types/:id/#{subcollection}") do

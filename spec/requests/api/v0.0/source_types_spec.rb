RSpec.describe("v0.0 - SourceTypes") do
  let(:attributes)      { {"name" => "test_name", "product_name" => "Test Product", "vendor" => "TestVendor"} }
  let(:collection_path) { "/api/v0.0/source_types" }

  describe("/api/v0.0/source_types") do
    context "get" do
    ...
    context "post" do
      it "success: with valid body" do
        ...
        expect(response).to have_attributes(
          :status => 201,
          :location => "http://www.example.com/api/v0.0/source_types/#{response.parsed_body["id"]}",
          :parsed_body => a_hash_including(attributes)
        )
      end
  ...
  describe("/api/v0.0/source_types/:id") do
  ...
  describe("/api/v0.0/source_types/:id/sources") do
